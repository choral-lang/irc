...
public class IrcClientStep@(Client, Server)
        implements LoopsConsumer@(Client, Server)<Message> {
    private LoopsLoop@Server<Message> serverLoop;
    ...
    public IrcClientStep(LoopsLoop@Server<Message> serverLoop,
     ...
     */
    public void accept(Message@Client msg) {
        ...
        switch (cmd) {
            case PING -> {
                ...
                if (!serverState.isRegistered(clientId)) {{
                    serverLoop.add(ServerUtil@Server.forwardNumeric(
